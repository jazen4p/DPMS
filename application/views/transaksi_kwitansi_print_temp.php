  ...
  <?php if(isset($check_all)){foreach($check_all as $row){?>
   <div id="header">
      ...
      <p>
        ...
        HP: 0813.2793.5678 <span style="text-align: right; padding-left: 450px; color: red">NO.KWT.<?php echo $row->kode_perumahan."1".substr("000{$row->id_keuangan}", -3)?></span><br/>
      </p>
    <hr>
   </div>
   <div id="footer">
   ...
   <div id="content">
                 ...
                 <tbody>
                        <?php foreach($ppjb as $row2){?>
                            ...
                            <tr>
                                ...
                                <th style="padding-left: 20px">:</th>
                                <th><?php echo terbilang($row->pembayaran)?> Rupiah</th>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                ...
                                <th style="padding-left: 20px">:</th>
                                <?php $temp=1; foreach($this->db->get_where('rumah', array('kode_perumahan'=>$row->kode_perumahan, 'tipe_produk'=>$row->tipe_produk, 'no_psjb'=>$row->no_ppjb))->result() as $row4){
                                ...
                                <th><?php echo $row->tahap?> (Kwitansi Cicilan Sementara) <br>Perumahan <?php echo $row5->nama_perumahan." ".$temp?> Unit Type <?php echo $row2->tipe_rumah?> Blok <?php echo $row2->no_kavling;?> Jl. Desa Durian Kec. Sungai Ambawang Kubu Raya</th>
                            </tr>
                        <?php }?>
                 </tbody>
             </table>
     ...
     <p>
        <table style="width: 100%; table-layout: fixed">
          <tr>
            <td>
              <span style="width: 120px; border: 1px solid; padding: 5px; margin: 20px; margin-left: 40px; font-size: 18px; font-weight: bold">Rp. <?php echo number_format($row->pembayaran)?></span>                
            </td>
            <td style="text-align: center">
                Pontianak, <?php echo date('d', strtotime($row->tanggal_bayar))." ".ubahBulan(date('M', strtotime($row->tanggal_bayar)))." ".date('Y', strtotime($row->tanggal_bayar))?>
            </td>
          </tr>
          <tr>
            <td></td>
                 ...
                 <tbody>
                        <?php foreach($this->db->get_where('ppjb', array('no_psjb'=>$row->no_ppjb))->result() as $row2){?>
                            ...
                            <tr>
                                <th style="white-space: nowrap;">Untuk pembayaran</th>
                                <th>:</th>
                                <?php $temp=1; foreach($this->db->get_where('rumah', array('kode_perumahan'=>$row->kode_perumahan, 'tipe_produk'=>$row->tipe_produk, 'no_psjb'=>$row->no_psjb))->result() as $row4){
                                    if($row4->no_psjb==$row->no_psjb){ $temp = $temp + 1;}?>
                                <?php } foreach($this->db->get_where('perumahan', array('kode_perumahan'=>$row->kode_perumahan))->result() as $row5){ }?>
