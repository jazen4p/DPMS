        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">Skema Kontrol Budget</li>
            </ol>
          </div>
        </div>
            ...
            <div class="card">
              ...
              <div class="card-body">
                <div class="row">
                  <div class="col-12">
                    <?php if(isset($revisi)){?>
                        <form action="<?php echo base_url()?>Dashboard/edit_kontrol_budget" method="POST">
                        ...
                        <tbody>
                            ...
                            <tr style="background-color: yellow">
                                <td></td>
                                <td style="white-space: nowrap"><?php echo $row->kode_induk?></td>
                            ...
                            // $query = $this->db->get_where('keuangan_kontrol_budget', array('kode_induk'=>$row->kode_induk, 'kode_perumahan'=>$kode_perumahan))->result();
                            foreach($query as $row1){?>
                                <tr>
                                <td><?php echo $no;?></td>
                                    <td style="white-space: nowrap"><?php echo $row1->kode_pengeluaran?>
                                        <input type="hidden" name="kode_pengeluaran[]" value="<?php echo $row1->kode_pengeluaran?>">
                                        <input type="hidden" name="kode_induk[]" value="<?php echo $row1->kode_induk?>">
                                        <!-- <input type="hidden" name="id_induk[]" value="<?php echo $row1->id_induk?>">
                                        <input type="hidden" name="pos[]" value="<?php echo $row1->pos?>"> -->
                                    </td>
                                    <td style="white-space: nowrap"><?php echo $row1->nama_pengeluaran?><input type="hidden" name="nama_pengeluaran[]" value="<?php echo $row1->nama_pengeluaran?>"></td>
                                    <!-- <td><?php echo $row1->kategori_induk?><input type="hidden" name="kategori_induk[]" value="<?php echo $row1->kategori_induk?>"></td> -->
                                    <?php 
                                    $ts = $this->db->get_where('keuangan_kontrol_budget', array('kode_pengeluaran'=>$row1->kode_pengeluaran, 'kode_perumahan'=>$kode_perumahan));
                                    if($ts->num_rows() > 0){
                                      foreach($ts->result() as $tst){?>
                                        <td><input type="number" step="0.01" min=0 name="volume[]" class="form-control" id="volume<?php echo $no?>" value="<?php echo $tst->volume?>"></td>
                                        <td><input type="text" name="satuan[]" class="form-control" value="<?php echo $tst->satuan?>" id="satuan<?php echo $no?>"></td>
                                        <td><input type="number" step="0.01" min=0 name="harga_satuan[]" class="form-control" id="hargaSatuan<?php echo $no?>" value="<?php echo $tst->harga_satuan?>"></td>
                                        <td><input type="number" name="sub_jumlah[]" class="form-control" id="subJumlah<?php echo $no?>" value="<?php echo $tst->sub_jumlah?>" readonly></td>
                                    <?php }} else {?>
                                        <td><input type="number" step="0.01" min=0 name="volume[]" class="form-control" id="volume<?php echo $no?>" value=""></td>
                                        <td><input type="text" name="satuan[]" class="form-control" value="" id="satuan<?php echo $no?>"></td>
                                        <td><input type="number" step="0.01" min=0 name="harga_satuan[]" class="form-control" id="hargaSatuan<?php echo $no?>" value=""></td>
                                        <td><input type="number" name="sub_jumlah[]" class="form-control" id="subJumlah<?php echo $no?>" value="" readonly></td>
                                    <?php }?>
                                </tr>
                            <?php $no++; } }?>
                        </tfoot>
                    ...
                    <?php }else {?>
                    <form action="<?php echo base_url()?>Dashboard/add_kontrol_budget" method="POST">
                        <!-- <div class="form-group row">
                            <label class="col-sm-2">Perumahan</label>
                            <select name="kode_perumahan" class="form-control col-sm-10" required>
                                <option value="" disabled selected>-Pilih-</option>
                        ...
                        <tbody>
                            ...
                            foreach($query as $row1){?>
                                ...
                                <td><?php echo $no;?></td>
                                    ...
                                    <td style="white-space: nowrap"><?php echo $row1->nama_pengeluaran?><input type="hidden" name="nama_pengeluaran[]" value="<?php echo $row1->nama_pengeluaran?>"></td>
                                    <!-- <td><?php echo $row1->kategori_induk?><input type="hidden" name="kategori_induk[]" value="<?php echo $row1->kategori_induk?>"></td> -->
                                    <td><input type="number" step="0.01" min=0 name="volume[]" class="form-control" id="volume<?php echo $no?>"></td>
                                    <td><input type="text" name="satuan[]" class="form-control"></td>
                                    <td><input type="number" step="0.01" min=0 name="harga_satuan[]" class="form-control" id="hargaSatuan<?php echo $no?>"></td>
                                    <td><input type="number" name="sub_jumlah[]" class="form-control" id="subJumlah<?php echo $no?>" readonly></td>
                                </tr>
                            <?php $no++; } }?>
                        </tfoot>
                        </table>
