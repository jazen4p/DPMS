          ...
          <div class="col-12">
            ...
            <div class="card">
                <div class="card-header row">
                    ...
                    <div class="col-md-9">
                      <a href="<?php echo base_url()?>Dashboard/rekap_pemakaian_bahan?id=<?php echo $kode_perumahan?>" class="btn btn-outline-info btn-flat">Kembali</a>
                    </div>
                    <div class="col-md-3">
                ...
                <div class="card-body">
                    ...
                    <div class="col-12">
                        <div style="text-align: center; font-size: 12px">
                            <h6><b>REKAP PEMAKAIAN BAHAN PER UNIT</b></h6>
                            <h6>PROYEK <?php echo strtoupper($nama_perumahan)?></h6>
                        </div>
                        <div>
                          <form action="<?php echo base_url()?>Dashboard/update_pengerjaan" method="POST">
                            <table>
                                <tr>
                                    <td>Unit Kavling</td>
                                    <td style="padding-left: 50px">: 
                                      <?php if($kategori == "prasarana"){
                                        foreach($this->db->get_where('kawasan', array('id_kawasan'=>$id))->result() as $std){
                                          echo $std->nama;
                                        }
                                      } else {
                                        echo $no_unit;
                                    ...
                                    <tr>
                                        <td>Mulai Pengerjaan</td><td style="padding-left: 50px">: <input type="date" name="awal" value="<?php echo date('Y-m-d', strtotime($awal))?>"></td>
                                    </tr>
                                    <tr>
                                        <td>Proyek Selesai</td><td style="padding-left: 50px">: <input type="date" name="akhir" value="<?php echo date('Y-m-d', strtotime($akhir))?>"></td>
                                    </tr>
                                    <tr>
                                        <td>
                        ...
                        <?php 
                            $this->db->group_by('tgl_arus');
                            $ts = $this->db->get_where('logistik_arus_stok', array('jenis_arus'=>"keluar", 'kategori'=>$kategori, 'no_unit'=>$no_unit));
                            // print_r($ts->result());
                        ?>
                                ...
                                foreach($this->db->get_where('produksi_master_data', array('kategori'=>"barang"))->result() as $row){?>
                                    <tr>
                                        ...
                                        foreach($ts->result() as $tsr){
                                            $tss = $this->db->get_where('logistik_arus_stok', array('nama_barang'=>$row->nama_data, 'tgl_arus'=>$tsr->tgl_arus, 'jenis_arus'=>"keluar", 'no_unit'=>$no_unit, 'kode_perumahan'=>$kode_perumahan));
                                            // print_r($tss->result());
                                            $ttl = 0;
                          ...
                          <table class="table table-bordered">
                            ...
                            <tbody>
                              <tr style="background-color: lightblue">
                                <?php foreach($this->db->get_where('rumah', array('kode_rumah'=>$no_unit, 'kode_perumahan'=>$kode_perumahan))->result() as $rmh){?>
                                  <td>Budget Yang Telah Ditentukan</td>
                                  <td style="text-align: right"><?php echo "Rp. ".number_format($rmh->budget_bahan)?></td>
                                <?php }?>
                              </tr>
                              ...
                              <tr style="background-color: pink">
                                <td>Sisa Budget</td>
                                <td style="text-align: right"><?php echo "Rp. ".number_format($rmh->budget_bahan - $ttl2)?></td>
                              </tr>
                            </tbody>
                          </table>
                        <?php } else {?>
