          ...
          <div class="col-12">
            <div class="card">
              <div class="card-header">
                <a href="<?php echo base_url()?>Dashboard/bast_dev_kon_management?id=<?php echo $id?>" class="btn btn-success btn-sm">Kembali</a>
              ...
              <div class="card-body">
                <table id="example2" class="table table-bordered table-striped" style="font-size: 13px">
                  ...
                  <?php foreach($check_all->result() as $asw){
                    // print_r($this->db->get_where('kbk', array('id_kbk'=>$asw->id_kbk))->result());
                    foreach($this->db->get_where('kbk', array('id_kbk'=>$asw->id_kbk))->result() as $row){
                      foreach($this->db->get_where('kbk_termin', array('id_kbk'=>$row->id_kbk, 'tahap'=>"Termin 4"))->result() as $ters){
                        if($ters->realisasi_progress == 90){?>
                          <tr>
                            <td><?php echo sprintf('%03d', $row->no_kbk)."/KBK";?></td>
                            <!-- <td>
                              <?php foreach($this->db->get_where('spk', array('id_spk'=>$row->id_spk))->result() as $ppjb){
                                echo sprintf('%03d', $ppjb->no_spk)."/PPJB";;
                              }?>
                            </td> -->
                            ...
                            <td>
                              <?php 
                                $gt = $this->db->get_where('bast_konsumen', array('id_bast'=>$asw->id_bast));
                                // print_r($gt->num_rows());
                                if($gt->num_rows() < 1){?>
                                  <a href="<?php echo base_url()?>Dashboard/add_bast_dev_kon?id=<?php echo $asw->id_bast?>&kode=<?php echo $id?>" class="btn btn-sm btn-flat btn-outline-primary">Registrasi BAST</a>
                                <?php } else {
