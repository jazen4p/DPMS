...
?>
  <?php foreach($check_all as $row1){
    foreach($this->db->get_where('spk', array('unit'=>$row1->no_unit, 'kode_perumahan'=>$row1->kode_perumahan))->result() as $row){?>
   <div id="header">
      <?php 
   ...
   <div id="content" style="font-size: 12px">
      ...
      <div>
        Kepada Yth, <br>
        Pak. <?php echo $row1->nama_tukang?>
      </div> <br>

      <div>
        <?php foreach($this->db->get_where('ppjb', array('id_psjb'=>$row->id_ppjb))->result() as $ppjb){?>
          <div>Berdasarkan dokumen PPJB No. 1-<?php echo sprintf('%03d', $ppjb->no_psjb)?>/KBK/KBR/<?php echo $ppjb->kode_perusahaan."/".$ppjb->kode_perumahan."/".date('m', strtotime($ppjb->tgl_psjb))."/".date('y', strtotime($ppjb->tgl_psjb));?></div>
        <?php }?>
        <table style="padding-left: 10px">
          ...
          <tr>
            <td>Pekerjaan</td><td>:</td><td><?php echo ucfirst($row1->kategori)?></td>
          </tr>
          <tr>
            <td>Masa pelaksanaan</td><td>:</td><td><?php echo $row1->masa_kerja." Hari";?></td>
          </tr>
          <tr>
            <td>Kontrak pekerjaan</td><td>:</td><td><?php echo $row1->pekerjaan_ket?></td>
          </tr>
        </table>
      </div>
      <!-- <br>
      ...
      <br>
      <div>
        <div>Maka diperintahkan kepada Saudara untuk melaksanakan pembangunan sesuai kontrak proyek tersebut dengan ketentuan :</div> <br>
        <table style="padding-left: 10px">
          <tr>
            <td>Nilai Upah Kontrak</td><td>:</td><td><?php echo "Rp. ".number_format($row1->nilai_kontrak);?></td>
          </tr>
          <tr>
            <!-- <td>Batas akhir BAST pada tanggal</td><td>:</td><td><?php echo date('d F Y', strtotime($row->tgl_bast))?></td> -->
            <td>Mulai Kerja</td><td>:</td><td><?php echo date('d F Y', strtotime($row1->mulai_kerja))?></td>
          </tr>
          <tr>
            <!-- <td>Batas akhir BAST pada tanggal</td><td>:</td><td><?php echo date('d F Y', strtotime($row->tgl_bast))?></td> -->
            <td>Selesai Kerja</td><td>:</td><td><?php echo date('d F Y', strtotime($row1->selesai_kerja))?></td>
          </tr>
        </table>
      </div>
      <br>
      <div>
        Setiap perubahan pelaksanaan yang menyimpang dari Rencana dan spesifikasi harus dikoordinasikan dengan Bagian Arstiker penjualan / Marketing dan dimintakan persetujuan dengan konsumen.
      </div>
      <br>
      <div>
      ...
      <table style="table-layout: fixed; width: 100%">
        <tr>
            <td>Menyepakati, </td>
            <td></td>
            <td style="text-align: center"><?php echo "Kubu Raya, ".date('d', strtotime($row1->date_by))." ".ubahBulan(date('M', strtotime($row1->date_by)))." ".date('Y', strtotime($row1->date_by))?></td>
        </tr>
        <tr>
            <td>Pekerja</td>
        ...
        <tr>
            <td>
            <?php if($row1->tukang_sign != ""){?>
                <img src="./gambar/signature/kontrak_kerja/<?php echo $row1->tukang_sign?>" style="width: 200px; height: 115px">
            <?php } else {
                echo "<span><br><br><br><br></span>"; 
            }?>
            </td>
            <td style="text-align: center">
            <?php if($row1->manager_sign != ""){?>
                <img src="./gambar/signature/kontrak_kerja/<?php echo $row1->manager_sign?>" style="width: 200px; height: 115px">
            <?php } else {
                echo "<span><br><br><br><br></span>"; 
            }?>
            </td>
            <td style="text-align: center">
            <?php if($row1->dev_sign != ""){?>
                <img src="./gambar/produksi/<?php echo $row1->dev_sign?>" style="width: 200px; height: 115px">
            <?php } else {
                echo "<span><br><br><br><br></span>"; 
            }?>
            </td>
        ...
        <tr>
            ...
            <td style="text-align: center"><?php echo "Hendra Hartady"?></td>
