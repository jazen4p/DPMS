...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <div class="container-fluid">
      <div class="card">
        <div class="card-header">
          <h5>Riwayat Pembayaran & Pencetakan Struk (Lunas)</h5>
    ...
    <div class="container-fluid">
      <div class="card">
        <div class="card-header">
          <h5>Riwayat Pembatalan</h5>
          <?php if(isset($succ_msg1)){
            echo "<span style='color: green'>$succ_msg1</span>";   
          }?>
          <?php if(isset($err_msg)){
            echo "<span style='color: red'>$err_msg</span>";   
          }?>
        ...
        <!-- <form role="form" method="POST" action="<?php echo base_url()?>Kasir/edit_data_unit" enctype="multipart/form-data"> -->
          ...
          <div class="card-body">
            <div class="col-sm-12">
                <table class="table table-striped table-bordered" id="ex2" style="font-size: 13px; white-space: nowrap">
                  <thead>
                    <tr>
                      <th>No Struk</th>
                      <th>Tanggal Pembayaran</th>
                  ...
                  <tbody>
                    <?php foreach($check_all2->result() as $row1){?>
                        <tr>
                            <td><?php echo sprintf('%09d', $row1->id_struk)?></td>
                            <td><?php echo date('d-m-Y H:i', strtotime($row1->tgl_struk))?></td>
                            <td><?php echo $row1->kode_rumah?></td>
                            <td><?php echo $row1->nama_pemilik?></td>
                            <td><?php echo $row1->hp_pemilik?></td>
                            <td><?php echo "Rp. ".number_format($row1->grand_total)?></td>
                            <td><?php echo "Rp. ".number_format($row1->pembayaran)?></td>
                            <td><?php echo ucfirst($row1->jenis_pembayaran)." "?><?php foreach($this->db->get_where('bank', array('id_bank'=>$row1->id_bank))->result() as $banks){echo $banks->nama_bank;}?></td>
                            <td><?php echo $row1->status?></td>
                            <td>
                                <a href='<?php echo base_url()?>Kasir/print_struk?id=<?php echo $row1->id_struk?>' target="_blank" class="btn btn-sm btn-success"><i class="fa fa-file"></i> Print</a>
                                <button type="button" class="btn btn-info btn-sm" data-toggle="modal" data-target="#exampleModal" data-id="<?php echo $row1->id_struk?>"><i class="fa fa-list-alt"></i> Detail</button>
                                <!-- <a href="<?php echo base_url()?>Kasir/hapus_struk?id=<?php echo $row->id_struk?>" class="btn btn-danger btn-sm"><i class="fa fa-times"></i> Batal</a> -->
                            </td>
                        </tr>
                    <?php }?>
...
<script>
  function myFunction1(id) {
    ...
    if (r == true) {
      ...
      window.location.replace(base_url+"/riwayat_pembayaran");
    }
  ...
  $('#exampleModal').on('show.bs.modal', function (e) {
    ...
    $.ajax({
        type: "POST",
        url: "<?php echo base_url()?>Kasir/get_riwayat_struk_cicil_lunas",
        data: { country : myRoomNumber } 
    }).done(function(data){
